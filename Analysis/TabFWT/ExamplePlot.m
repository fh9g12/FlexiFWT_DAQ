...
indicies = indicies & [MetaData.AoA]==5;
indicies = indicies & [MetaData.Velocity]>24;
indicies = indicies & [MetaData.Velocity]<25;
indicies = indicies & [MetaData.TabAngle]==-10;

RunData = MetaData(indicies);


data = LoadRunNumber(RunData(1).RunNumber);

figure(1)
clf;
plot(data.daq.t,movmean(data.daq.strain.v,10))
ylabel('Root Bending Strain Gauge Voltage [V]')
xlabel('Time [s]')
grid minor
