...
config = ConfigMeta.CreateMeta('free10')...
    .set_T(0.12).set_enviroment('sim','--');

loc = [2.5,35,110,176,288];
label = ["A","B","C","D","E"];
...
[left_data,right_data,v] = deal(zeros(length(bins),length(filtData)));
...
yyaxis left
h = plot(bins,nanmean(right_data,2),'-');
h.LineWidth =2;
hold on
...
filename = '/Users/fintan/Desktop/Figures/fwt_orientation_sim.png';
